...
inline bool operator!=(const TileCoord& l, const TileCoord& r) { return l.x != r.x || l.y != r.y; }
inline bool operator==(const TileCoord& l, const TileCoord& r) { return l.x == r.x && l.y == r.y; }

inline TileCoord operator+(const TileCoord& l, const TileCoord& r) { return { l.x + r.x, l.y + r.y }; }
inline TileCoord operator-(const TileCoord& l, const TileCoord& r) { return { l.x - r.x, l.y - r.y }; }
 
inline TileCoord operator*(const TileCoord& l, const int& r) { return { l.x * r, l.y * r }; }
inline TileCoord operator/(const TileCoord& l, const int& r) { return { l.x / r, l.y / r }; }
